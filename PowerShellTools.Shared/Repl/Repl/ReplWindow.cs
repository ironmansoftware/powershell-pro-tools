...
namespace PowerShellTools.Repl
{
    ...
    internal sealed class ReplWindow : ToolWindowPane, IOleCommandTarget, IReplWindow, IVsFindTarget
    {
        ...
        protected override void OnCreate()
        {
            ...
            textView.ViewportWidthChanged += (sender, args) =>
            {
                try
                {
                    if (PowerShellDebuggingService.Runspace?.RunspaceAvailability == RunspaceAvailability.Available)
                    {
                        using (var ps = PowerShell.Create())
                        {
                            ps.Runspace = PowerShellDebuggingService.Runspace;
                            ps.AddScript($"$Host.UI.RawUI.BufferSize = [System.Management.Automation.Host.Size]::new({GetRawHostBufferWidth()},0)");
                            ps.Invoke();
                        }
                    }
                }
        ...
        public void ClearScreen(bool insertInputPrompt)
        {
            if (!CheckAccess())
            {
        ...
        private void Copy()
        {
            ...
            foreach (var projectionSpan in TextView.Selection.SelectedSpans)
            {
                ...
                text.Append(projectionSpan.Snapshot.GetText(projectionSpan.Span));
        ...
        private sealed class CommandFilter : IOleCommandTarget
        {
            ...
            private VsKeyInfo GetVsKeyInfo(IntPtr pvaIn, VSConstants.VSStd2KCmdID commandID)
            {
                ...
                bool capsLockToggled = Keyboard.IsKeyToggled((System.Windows.Input.Key)Key.CapsLock);
                bool numLockToggled = Keyboard.IsKeyToggled((System.Windows.Input.Key)Key.NumLock);
                ...
                Key key = (Key)KeyInterop.KeyFromVirtualKey(virtualKey);
        ...
        public int QueryStatus(ref Guid pguidCmdGroup, uint cCmds, OLECMD[] prgCmds, IntPtr pCmdText)
        {
            ...
            if (pguidCmdGroup == GuidList.guidReplWindowCmdSet)
            {
                ...
                {
                    case PkgCmdIDList.cmdidOpenConsole:
            ...
            if (pguidCmdGroup == VSConstants.VSStd2K)
            {
                switch ((VSConstants.VSStd2KCmdID)nCmdID)
                {
                    ...
                    case VSConstants.VSStd2KCmdID.CANCEL:
                        _historySearch = null;
                        var ic = (IntellisenseController)TextView.Properties.GetProperty(typeof(IntellisenseController));
                        if (ic.CompletionBroker.IsCompletionActive(TextView))
                        {
                            ic.CompletionBroker.DismissAllSessions(TextView);
                        }
                        else
                        {
                            Cancel();
